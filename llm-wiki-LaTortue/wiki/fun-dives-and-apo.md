# Fun Dives And Apo Island

**Summary**: Current certified-diver prices, inclusions, and Apo Island day-trip content.

**Sources**: diving-fun-dives.html, fr/diving-fun-dives.html, diving-apo-trips.html, fr/diving-apo-trips.html, index.html, fr/index.html.

**Last updated**: 2026-06-23.

---

The fun-dives page targets certified divers with guided coastal dives, night dives, and Apo Island trips (source: diving-fun-dives.html; source: fr/diving-fun-dives.html). The homepage summarizes fun dives as daily Dauin coastal diving and weekly Apo trips, with 3-4 shore-entry dives per day starting at PHP 1,900 and Apo Island boat day package at PHP 6,200 (source: index.html; source: fr/index.html).

## Published Certified-Diver Prices

The current fun-dive price list publishes Fun Dive at PHP 1,900, Night Dive at PHP 2,200, Refresher Dive at PHP 2,700, Apo Island Trip at PHP 6,200, private dive master/instructor at +PHP 500, private Apo guide at +PHP 2,500, and Nitrox Tank / 15 L Air Tank at +PHP 200. Snorkeling in Apo Island is listed separately at PHP 2,500 including food, marine fees, and snorkel gear, with no snorkeling guide included (source: diving-fun-dives.html; source: fr/diving-fun-dives.html).

The fun-dive page states that prices include marine fees, gear, dive master/instructor, and transportation to the dive site (source: diving-fun-dives.html). The French version has equivalent pricing and inclusions for certified divers (source: fr/diving-fun-dives.html).

## Fun-Dive Experience

The page frames diving as confidence-building, safe, relaxed, and professional, with Dauin's underwater environment as the core setting (source: diving-fun-dives.html; source: fr/diving-fun-dives.html). The fun-dive gallery uses many square-cut images of Dauin shore dives and a lightbox interaction (source: diving-fun-dives.html; source: fr/diving-fun-dives.html; source: assets/site.js).

## Apo Island Day Trips

The Apo Island page describes a boat trip with three dives, meals on board, complete equipment, marine fees, meal, and snacks included in an all-in price (source: diving-apo-trips.html). The French Apo page mirrors the same trip structure and highlights (source: fr/diving-apo-trips.html).

Current Apo highlight sites are Rockpoint East/West, Chapel, Coconut Point, Kan-uran, Largahan, Cogon, and Katipanan (source: diving-apo-trips.html; source: fr/diving-apo-trips.html). The menu and homepage route Apo traffic to the dedicated Apo Island trips page (source: menu.html; source: fr/menu.html; source: index.html; source: fr/index.html).

## Related Pages

- [[diving-courses]]
- [[dive-sites]]
- [[rooms-and-dining]]
- [[team-and-contact]]
